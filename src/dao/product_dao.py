...
class ProductDAO:
    ...
    def create(self, name: str, sku: str, price: float, stock: int = 0, category: str | None = None) -> Dict:
        payload = {"name": name, "sku": sku, "price": price, "stock": stock}
        if category:
            payload["category"] = category
        self.db.table("products").insert(payload).execute()
    ...
    def delete(self, prod_id: int) -> Optional[Dict]:
        before = self.db.table("products").select("*").eq("prod_id", prod_id).limit(1).execute()
    ...
    def list(self, limit: int = 100, category: str | None = None) -> List[Dict]:
        q = self.db.table("products").select("*").order("prod_id").limit(limit)
        if category:
            q = q.eq("category", category)
        resp = q.execute()
        return resp.data or []
